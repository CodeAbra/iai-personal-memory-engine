"""Bench-only kill-switch guard for the multi_seed gate.

``IAI_MCP_MULTI_SEED_OFF`` is an additive env-var toggle mirroring the
existing ``IAI_MCP_EXACT_AUTHORITY_OFF`` precedent (``core/__init__.py:120,
388``). Default (env unset) fires the ``multi_seed`` trace mark
unconditionally on every recall call; setting the toggle to ``"true"`` skips
the widen step and its trace_mark entirely, independent of cue confidence.
...
@pytest.fixture(autouse=True)
def _clear_multi_seed_toggle(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_multi_seed_fires_by_default(driver, _store, monkeypatch):
    """Test 1: env unset (default) -> multi_seed mark present."""
    ...
    assert "multi_seed" in marks, f"expected 'multi_seed' with the toggle unset, got {sorted(marks)}"


@pytest.mark.parametrize("driver", ["stdlib", "lilli"])
def test_multi_seed_off_disables_multi_seed(driver, _store, monkeypatch):
    """Test 2: IAI_MCP_MULTI_SEED_OFF=true -> multi_seed absent; response stays well-formed."""
    monkeypatch.setenv("IAI_MCP_RECALL_TRACE", "1")
