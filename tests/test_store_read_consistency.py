...
# The two WAL-cross-visibility tests below assert SQLite WAL cross-visibility
# between two concurrently-open connections; the native engine uses
# independent pagers per open store and has no multi-connection snapshot
# analog (and read_consistency_interval has no production consumer there).
# The native engine's durable at-rest persistence across reopen is proven by
# test_store_raw.py::test_open_store_raw_lilli_returns_engine_conn. Each such
# test therefore sets the legacy driver itself before opening any store.
...
def test_reader_with_strong_consistency_sees_writer_insert(tmp_store_env, monkeypatch):
...
def test_default_connection_is_snapshot_pinned(tmp_store_env, monkeypatch):
    monkeypatch.setenv("LILLI_STORAGE_DRIVER", "stdlib")
    from iai_mcp.store import MemoryStore
