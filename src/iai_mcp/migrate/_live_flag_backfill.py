...
def migrate_live_flag_backfill(
    ...
    (``live IS NULL``): ``live = 1`` when ``tombstoned_at`` is unset, else
    ``live = 0``. Rows written after the change already carry the correct
    value and are skipped. A fresh-migrated store's rows land with ``live``
    NULL (the copied ``tombstoned_at`` predates the column), so this backfill
    runs on the FIRST open of a fresh-migrated store — the boot-migration
    sequence is unconditional (no stamp short-circuit before the first run),
    so the live-rows recall query rides the index from the first recall.
    ...
    # at write time, so the backfilled column agrees with the legacy
    # `tombstoned_at IS NULL` predicate for every row, byte-identically.
    with db._conn_lock:
