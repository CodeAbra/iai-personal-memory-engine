...
def _generate_token() -> str:
    ...
    The file is created empty and ACL-restricted BEFORE the secret is written,
    so no window exists where the token sits under the default inherited ACL.
    """
...
async def start_ipc_server(
    ...
    *,
    ...
    if IS_WINDOWS:
        ...
        _kwargs: dict[str, Any] = {"limit": limit} if limit is not None else {}
        server = await asyncio.start_server(
            authenticated_handler, "127.0.0.1", 0, **_kwargs
        )
