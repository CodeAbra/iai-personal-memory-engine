...
- Your environment (operating system and version, Python version).
...
- The host machine is trusted. An attacker with local code execution as the user can read the encryption key (`~/.iai-mcp/.crypto.key`) and the unlocked store.
...
- Defects requiring the user to manually set insecure file permissions on `~/.iai-mcp/.crypto.key`.
...
- The key is derived from a user-provided passphrase using a standard KDF and stored at `~/.iai-mcp/.crypto.key` with mode 0600.
