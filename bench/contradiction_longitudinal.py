...
DEFAULT_STORE_DIR = "/tmp/iai-mcp-bench/store"
    ...
    try:
        ...
        for entry, emb in zip(entries, embeddings):
            ...
            rec = MemoryRecord(
                ...
                created_at=now,
                updated_at=now,
                tags=["bench-contradiction", f"role:{entry.role}",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Contradiction-longitudinal falsifiability bench (reference implementation)",
    )
